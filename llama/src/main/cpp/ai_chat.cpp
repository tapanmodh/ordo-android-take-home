...
static int last_prompt_token_count = 0;
static int last_generated_token_count = 0;

static int64_t last_prefill_time_us = 0;
static int64_t generation_start_us = 0;
static int64_t first_token_time_us = 0;
static int64_t generation_end_us = 0;
...
Java_com_arm_aichat_internal_InferenceEngineImpl_resetConversationNative(
    ...
    if (g_context == nullptr) {
        LOGw("%s: No active context to reset", __func__);
        return;
    }
    ...
    last_prefill_time_us = 0;
    generation_start_us = 0;
    first_token_time_us = 0;
    generation_end_us = 0;

    LOGi("%s: Conversation context reset", __func__);
}
...
Java_com_arm_aichat_internal_InferenceEngineImpl_processUserPrompt(
    ...
    // Use actual number of tokens that will be processed.
    last_prompt_token_count =
            (int) user_tokens.size();

    const int64_t prefill_start_us =
            ggml_time_us();
    ...
    last_prefill_time_us =
            ggml_time_us() - prefill_start_us;

    // Reset generation benchmark state
    last_generated_token_count = 0;
    first_token_time_us = 0;
    generation_end_us = 0;

    // Generation begins after prefill completes.
    generation_start_us =
            ggml_time_us();

    // Advance using the actual processed token count.
    current_position +=
            last_prompt_token_count;
...
Java_com_arm_aichat_internal_InferenceEngineImpl_generateNextToken(
    ...
    if (current_position >= stop_generation_position) {
        ...
        // Finalize decode timing.
        generation_end_us = ggml_time_us();
    ...
    // This represents one actual generated model token.
    last_generated_token_count++;

    // Record first-token timing only once.
    if (first_token_time_us == 0) {
        first_token_time_us = ggml_time_us();
    }
...
extern "C"
JNIEXPORT jstring JNICALL
Java_com_arm_aichat_internal_InferenceEngineImpl_getLastGenerationStats(
        JNIEnv *env,
        jobject /*unused*/
) {
    const double prefill_seconds =
            last_prefill_time_us / 1000000.0;

    const double first_token_decode_seconds =
            first_token_time_us > generation_start_us
                    ? (first_token_time_us - generation_start_us) / 1000000.0
                    : 0.0;

    // This is the TTFT that the user effectively experiences
    // from prompt processing through first generated token.
    const double ttft_seconds =
            prefill_seconds + first_token_decode_seconds;

    const double generation_seconds =
            generation_end_us > generation_start_us
                    ? (generation_end_us - generation_start_us) / 1000000.0
                    : 0.0;
    ...
    const double decode_tps =
            decode_seconds > 0.0 &&
                    last_generated_token_count > 0
                    ? last_generated_token_count / decode_seconds
                    : 0.0;

    const double prefill_tps =
            prefill_seconds > 0.0
                    ? last_prompt_token_count / prefill_seconds
                    : 0.0;
